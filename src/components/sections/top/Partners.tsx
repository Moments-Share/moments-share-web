...
import { roles, DAYS } from "@/lib/recruit";
...
const paths = [
  {
    ...
    // 「ガクチカ」は就活中の学生が毎日使っている言葉。
    // こちらの言葉（PM・企画・実行）ではなく相手の言葉で書くと、
    // 自分に向けられた募集だと分かる。
    // 「バイトじゃない」は、バイトを下に見る言い方にしないための形。
    // 週2〜3日は lib/recruit.ts の条件そのまま。条件を直せばここも変わる
    body: `有償の長期インターン。せっかくなら、バイトじゃないガクチカを。${DAYS}から。`,
    href: "/student-internship",
    action: "インターンに参加する",
  },
  {
    title: "フリーランス・複業",
    // どんな人が居るかを職種で書く。個人名・肩書き・写真は出さない
    // （どなたが関わっているかは、こちらから公表しない約束）。
    // 職種を並べるだけで「自分と同じような人が居る」ことは伝わる。
    //
    // 「ママさん」ではなく「子育て中の方」にしてある。
    // 子育てをしながら働きたい父親も同じ層にいるので、
    // 性別を書くと片方が「自分向けではない」と受け取る。
    // /people/ の書き方（専門人材・副業・子育て中・学生…）とも揃う
    body: "ライター、エンジニア、デザイナーをはじめ、子育て中の方など様々なメンバーが一緒に働いています。",
    href: "/contact",
