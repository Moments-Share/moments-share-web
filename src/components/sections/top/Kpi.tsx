      ...
      <div className="mx-auto max-w-[1400px]">
        <Reveal>
          ...
          <p className="mt-5 max-w-[32em] text-[15px] leading-[2] text-charcoal/70 md:text-[16px]">
            Visionの実現に向けて、私たちが大切にしている指標です。
          </p>
        </Reveal>

        <Reveal delay={0.1}>
          <div className="mt-12 grid grid-cols-1 border-t border-charcoal/15 md:mt-16 md:grid-cols-4 md:border-b">
            {indicators.map((item) => (
              <div
                key={item.label}
                className="border-t border-charcoal/15 py-8 first:border-t-0 md:border-t-0 md:border-l md:px-8 md:py-10 md:first:border-l-0 md:first:pl-0"
              >
                <div className="flex items-baseline gap-2">
                  <span
                    className="font-bold leading-none tracking-[-0.03em] tabular-nums text-green"
                    style={{ fontSize: "clamp(52px, 6vw, 84px)" }}
                  >
                    {item.value}
                  </span>
                  <span className="text-[15px] font-bold text-charcoal/70">{item.unit}</span>
                </div>
